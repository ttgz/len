...
router.post('/import-excel-variants', upload.single('file'), async (req, res) => {
    try {
        if (!req.file) {
            return res.status(400).json({ error: 'Chưa chọn file Excel' });
        }
        ...
        worksheet.eachRow((row, rowNumber) => {
            ...
            const [stt, name, price, product_id] = row.values.slice(1); // bỏ index 0
            imported.push({ name, price, product_id });
        });

        // Lưu vào DB (giả sử bạn dùng Sequelize)
        for (const item of imported) {
        
            await Variant.create(item);
        }
